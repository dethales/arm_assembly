...
definidos como word na área de dados no formato binário. Os dados deverão ser alinhados.
Não se esquecer de usar comandos de desvios para o caso de encontrar o maior número.
...
6. Identifique no seu projeto (Laboratório 4) a diferença entre acessar endereços nos
...
7. Identifique ainda no projeto, e com Memory habilitado através do Menu View, o valor
impresso no item 3 na Memória Principal, assim com o endereço no qual foi armazenado,
exemplo Figura 2. Observem que o valor e o endereço estão em hexadecimais.
8. Ainda na opção MemoryView, Figura 2, podemos comutar entre as exibições do tamanho da
palavra (Word Size) entre 8 bit, 16 bit ou 32 bit.
...
de carro encontre e imprime no stdout o menor de dois números binários
(menor_valor_binario).
2. Em seguida, imprime uma string de retorno de carro.
3. Após isto, imprima o menor valor encontrado menor_valor_binario, mas deslocando o
valor do número a direita de um bit.
4. Em seguida, imprime uma string de retorno de carro.
5. Agora imprime um valor qualquer no formato binário e em seguida faça um loop repetido
cinco vezes e a cada loop deslocar este valor a direita de um bit e imprimir. O que você
observou? Qual conclusão encontrou? Em que situação pode utilizar esta propriedade de
...
.data
num1: .word 0b00101001
num2: .word 0b00100001
num3: .word 0b00001101
